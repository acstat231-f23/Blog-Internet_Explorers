...
api_key <- "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"
...
send_prompt <- function(prompt) {
  ...
  if (http_status(response)$category == "success") {
    ...
    if (length(result$choices) > 0 && length(result$choices[[1]]$message) > 0) {
    ...
    cat("Error in API request: ", content(response, "text"), "\n")
    stop()
  }
}
print(2)
prompt_response <- send_prompt("write me a 4 line Frost Poem")
...
OilImports <- read_excel(file_path_imports)
OilExports <- read_excel(file_path_exports)
