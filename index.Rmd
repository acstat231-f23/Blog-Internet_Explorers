...
Search this table to find discrete values for imports and exports of different nations and groups of nations. Individual years can also be searched. 
...
combined_df %>%
  ...
  datatable(
    ...
    options = list(
      ...
      autoWidth = TRUE,
      columnDefs = list(
        list(targets = c(1,2), searchable = TRUE),  # 2 is the index of the 'Year' column
        list(targets = c(3,4), searchable = FALSE)  # Disable search for other columns
      )
    )
  )
```
...
OilExports %>%
  ...
  datatable(
    colnames = c("Country", "Year", "Exports"),
    filter = 'top',
    options = list(
      pagelength = 10,
      autoWidth = TRUE,
      columnDefs = list(
        ...
        list(targets = c(0, 1), searchable = FALSE)  # Disable search for other columns
      )
    )
  )
```
